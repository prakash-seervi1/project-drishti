...
class CrowdNotificationAgent(BaseAgent):
    def __init__(self):
        ...
        super().__init__("notification", memory, tools, comms)
    ...
    def update_structured_context(self, session_id, event, notification, prev_context):
        ...
        if "zone" in event_content.lower():
            ...
            if match:
                ...
                new_context["last_intent"] = "zone_notification"
        if "incident" in event_content.lower():
            new_context["last_intent"] = "incident_notification"
        structured_event = {
    ...
    def handle_event(self, event, session_id="default"):
        ...
        self.memory.save_context(session_id, notification_entry)
        self.update_structured_context(session_id, event, notification, structured_context)
        return notification
