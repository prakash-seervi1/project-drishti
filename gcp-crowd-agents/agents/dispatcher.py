...
class ResponseDispatcherAgent(BaseAgent):
    def __init__(self):
        memory = FirestoreMemory("dispatcher")
        tools = {
            "llm": query_gemini,
        ...
        comms = PubSubComms("dispatcher")
        super().__init__("dispatcher", memory, tools, comms)
    ...
    def update_structured_context(self, session_id, event, dispatch, prev_context):
        ...
        if "zone" in event_content.lower():
            ...
            if match:
                ...
                new_context["last_intent"] = "zone_dispatch"
        if "incident" in event_content.lower():
            new_context["last_intent"] = "incident_dispatch"
        structured_event = {
    ...
    def handle_event(self, event, session_id="default"):
        ...
        prompt = (
            f"Recent dispatch events (long-term):\n{long_term_context}\n"
            f"Data snapshot:\n{context}\n"
            f"{structured_context_str}\n"
            f"Given these high-risk zone alerts and responder status updates, generate dispatch instructions for responders.\nAssistant:"
        )
        dispatch = self.tools["llm"](prompt)
        self.comms.publish({"type": "dispatch", "dispatch": dispatch, "context": context})
        dispatch_entry = {
            "type": "dispatch",
            "timestamp": datetime.datetime.utcnow().isoformat(),
            "content": dispatch
        }
        self.memory.save_context(session_id, dispatch_entry)
        self.update_structured_context(session_id, event, dispatch, structured_context)
        return dispatch
