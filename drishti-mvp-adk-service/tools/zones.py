...
def create_zone_tool(zone: Dict[str, Any]):
...
def get_zone_by_id_tool(zone_id: str):
    doc = db.collection("zones").document(zone_id).get()
    if not doc.exists:
        raise ValueError("Zone not found")
    return {"id": doc.id, **doc.to_dict()}

def update_zone_tool(zone_id: str, updates: Dict[str, Any]):
...
def delete_zone_tool(zone_id: str):
    doc_ref = db.collection("zones").document(zone_id)
    if not doc_ref.get().exists:
        raise ValueError("Zone not found")
    doc_ref.delete()
