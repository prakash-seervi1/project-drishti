...
export default function AISituationSummary() {
  ...
  useEffect(() => {
    ...
    setErrorAssessment('')
    setAssessment('')
    agentAPI.runAgent({ input: 'Give me a current assessment of the overall situation for all active incidents, zones, and responders. Summarize the key risks and status.' })
      .then(res => {
        // Prefer 'answer' key, fallback to previous keys
        if (res && res.answer) {
    ...
    setRecommendations('')
    agentAPI.runAgent({ input: 'Provide actionable, prioritized AI recommendations for safety, resource allocation, and incident response for the current event situation.' })
      .then(res => {
        // Prefer 'answer' key, fallback to previous keys
        if (res && res.answer) {
